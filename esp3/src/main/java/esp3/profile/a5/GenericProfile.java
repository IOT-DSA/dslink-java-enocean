...
import esp3.profile.a5.xml.Case;
...
public class GenericProfile extends Profile {
    ...
    public GenericProfile(String id, int func, int type, EnocianType enocianType) {
        ...
        for (int i=caseInfo.size(); i<getNumCases(); i++) {
    ...
    protected int getCaseFromTelegram(RadioPacket radio) {
    	return defaultCase();
    }
    ...
    protected int getNumCases() {
    	if (enocianType != null && enocianType.getCases() != null) {
    		return enocianType.getCases().size();
    	}
    	return 1;
    }
    	...
    	if (getNumCases() > 1) {
    		...
    		for (Case cas: enocianType.getCases()) {
    			if (cas!=null && cas.getDatafields() != null) {
    				for (DataField dataField : cas.getDatafields()) {
    					handleDatafieldPointInfo(dataField, caseNum);
    				}
    			}
    			caseNum += 1;
    		}
    	} else if (enocianType != null && enocianType.getDatafields() != null) {
            for (DataField dataField : enocianType.getDatafields()) {
                handleDatafieldPointInfo(dataField, 0);
            }
        }
    }
    
    private void handleDatafieldPointInfo(DataField dataField, int caseNum) {
    	if (!dataField.isLearn()) {
        	boolean isOut = false;
...
//        	if (name.contains("setpoint") || name.contains("set point") || name.contains("command")) isOut = true;
            ...
            if (items != null) {
                if (items.get(0).getScale() != null || items.size() != 2) {
                	caseInfo.get(caseNum).put(dataField.getShortcut(), new PointInfo(DataTypes.MULTISTATE, isOut));
                } else {
                	caseInfo.get(caseNum).put(dataField.getShortcut(), new PointInfo(DataTypes.BINARY, isOut));
                }
            } else {
            	caseInfo.get(caseNum).put(dataField.getShortcut(), new PointInfo(DataTypes.NUMERIC, isOut));
            }
        }
    }
